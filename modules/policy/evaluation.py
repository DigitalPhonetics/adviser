...
from utils.goal import Goal, Constraint
...
class ObjectiveReachedEvaluator(object):
    ...
    def get_final_reward(self, sim_goal : Goal, logging = True):
        ...
        #print(requests['name'])
        db_matches = self.domain.find_info_about_entity(entity_id=requests['name'], requested_slots=[constraint.slot for constraint in constraints])
        if db_matches:
            match = db_matches[0]
...
class PolicyEvaluator(Module):
    ...
    def __init__(self, domain : Domain, subgraph: dict = None, use_tensorboard=False,
        ...
        """
        super(PolicyEvaluator, self).__init__(domain, subgraph = subgraph, logger = logger)
        ...
        if use_tensorboard:
            self.writer = SummaryWriter(os.path.join('logs', experiment_name))
        else:
            self.writer = None
    ...
    def forward(self, dialog_graph, sim_goal : Goal = None, **kwargs) -> dict(writer=SummaryWriter):
        """ See `modules.module.forward` """
        # if epoch > self.epoch:
        #     self._end_epoch()
        #     self.epoch = epoch 
        #     self._start_epoch()
    ...
    def start_dialog(self, **kwargs):
        """ See `modules.module.start_dialog` """
    ...
    def start_epoch(self):
        """ Call this function between epochs """
    ...
    def end_epoch(self):
        """ Call this function after an epoch finisehd """
