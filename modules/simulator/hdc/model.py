...
from utils import UserAct, UserActionType, SysAct, SysActionType, Constraint, Goal, common
from utils.logger import DiasysLogger
from modules.simulator.simulator import UserSimulator
from utils.domain.domain import Domain


class HandcraftedUserSimulator(UserSimulator):
    ...
    def __init__(self, domain: Domain, logger: DiasysLogger = DiasysLogger()):
        super(HandcraftedUserSimulator, self).__init__(domain, logger=logger)
        ...
        self.receive_options = {SysActionType.Welcome: self._receive_welcome,
                                ...
                                SysActionType.Bye: self._receive_bye,
    ...
    def start_dialog(self, **kwargs):
        ...
        self.logger.dialog_turn("New goal has constraints {} and requests {}."\
            .format(self.goal.constraints, self.goal.requests))
    ...
    def receive(self, sys_act):
        ...
        if self.patience == 0:
            ...
            ignored_requests, ignored_requests_alt = self._check_system_ignored_request(self.last_user_actions, sys_act)
    ...
    def _receive_welcome(self, sys_act):
        """Processes a welcome action from the system."""
    ...
    def _receive_informbyalternatives(self, sys_act):
        """Processes an informbyalternatives action from the system."""
        # same as inform
    ...
    def _receive_request(self, sys_act):
        """Processes a request action from the system."""
    ...
    def _receive_requestmore(self, sys_act):
    ...
    def _receive_bad(self, sys_act):
        """Processes a bad action from the system."""
        # NOTE repeat last action, should never occur on intention-level as long no noise is used
        self._repeat_last_actions()

    def _receive_bye(self, sys_act):
        """Processes a bye action from the system."""
        # if goal is not fulfilled ignore, otherwise end dialog
        if self.goal.is_fulfilled():
            self._finish_dialog()
    ...
    def _check_informs(self, informed_constraints_by_system):
        ...
        for _constraint in informed_constraints_by_system:
            if self.goal.is_inconsistent_constraint(_constraint):
                ...
                self.agenda.push(UserAct(act_type=UserActionType.Inform, slot=_constraint.slot,\
    ...
    def _check_offer(self, offers, informed_constraints_by_system):
        ...
        if offers:
            if 'none' not in offers:
                            ...
                            if common.random.random() < self.parameters['usermodel']['ReqAlt']:
                                ...
                                for _action in self.goal.missing_informs:
                                    ...
                                    if Constraint(_action.slot, _action.value) not in informed_constraints_by_system:
                                        self.agenda.push(UserAct(act_type=UserActionType.Request, slot=_action.slot, value=None))
                                return True
                ...
                self.goal.reset()
                self.goal.missing_informs = [UserAct(act_type=UserActionType.Inform, slot=_constraint.slot, value=_constraint.value) for _constraint in self.goal.constraints]
...
class Agenda(object):
    ...
    def get_actions(self, num_actions):
        """Retrieves *num_actions* actions from the agenda.

        Args:
            num_actions: Amount of actions which will be retrieved from the agenda.
    ...
    def contains_action_of_type(self, act_type: UserActionType, consider_dontcare=True):
        ...
        Args:
            act_type: The action type (intent) for which the agenda will be checked.
            consider_dontcare: If set to True also considers actions for which the value is
    ...
    def get_actions_of_type(self, act_type=UserActionType, consider_dontcare=True):
        ...
        """
        return filter(lambda x: x.type == act_type\
    ...
    def remove_actions_of_type(self, act_type: UserActionType):
        ...
        Args:
            act_type: The action type (intent) which will be removed from the agenda.
    ...
    def remove_actions(self, act_type: UserActionType, slot: str, value: str = None):
        ...
        Args:
            act_type: The action type (intent) which will be removed from the agenda.
            slot: The action type (intent) which will be removed from the agenda.
            value: The action type (intent) which will be removed from the agenda.

        """
        if value is None:
            self.stack = list(filter(lambda x: x.type != act_type or x.slot != slot, self.stack))
        else:
            self.stack = list(filter(lambda x: x.type != act_type or x.slot != slot or\
                x.value != value, self.stack))

    def fill_with_requests(self, goal, exclude_name=True):
        ...
        Args:
            goal: The current goal of the (simulated) user for which actions will be pushed to the
                agenda.
            exclude_name: If True, an action requesting for the name is omitted and included
                otherwise.
