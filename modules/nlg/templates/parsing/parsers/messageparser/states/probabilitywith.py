...
from modules.nlg.templates.parsing.configuration import \
...
from modules.nlg.templates.parsing.parsers.messageparser.states.statelist import \
    ProbabilityWithState, ProbabilityState


class _ProbabilityWithDefaultTransition(DefaultTransition):
    def __init__(self):
        DefaultTransition.__init__(self, ProbabilityWithState())
...
class _ProbabilityEndTransition(TransitionWithoutAction):
    ...
    def perform_stack_action(self, stack: AutomatonStack, configuration: Configuration):
        ...
        stack.add_data(MessageComponent(MessageComponentType.PROBABILITY, float(current_string)))
        stack.remove_level()
...
class AcceptStateDescription(StateDescription):
    def __init__(self):
        StateDescription.__init__(self, AcceptState(), _InvalidAcceptTransition(), [
            TransitionWithAction(Configuration(AcceptState(), 'w'),
                                 Configuration(ProbabilityState(), 'w'))
        ])
