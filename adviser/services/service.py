			...
			if result:
				...
				for topic in pub_topics:
					...
					if topic in result:
						...
						domain = domain if isinstance(domain, str) else domain.get_domain_name().lower()
						topic_domain_str = f"{topic}.{domain}" if domain else topic
						self.logger.info(f" \n\n\n\n\nBLUBB2, {topic_domain_str}, {self._pub_topic_domains} \n\n\n\n\n")
						if topic in self._pub_topic_domains:
							self.logger.info(f" \n\n\n\n\nBLUBB, {domain}, {topic}, {topic_domain_str} \n\n\n\n\n")
							topic_domain_str = f"{topic}.{self._pub_topic_domains[topic]}" if len(self._pub_topic_domains[topic]) > 0 else topic
						asyncio.create_task(self._client.publish(topic_domain_str, {topic: result[topic]}))
...
class Service:
	...
	def _init(self): 
		...
		sub_callbacks = [
			(URI('_ctrl_dialog_start').uri, self.dialog_start),
			(URI('_ctrl_dialog_end').uri, self.dialog_end),
			(URI('_ctrl_dialog_exit').uri, self.dialog_exit)
		]

		domain_str = self.domain if isinstance(self.domain, str) else self.domain.get_domain_name().lower()
		if len(domain_str) > 0:
		...
		for func_name in dir(self):
			...
			if hasattr(func_inst, "pubsub"):
				...
				for topic in getattr(func_inst, "sub_topics") + getattr(func_inst, 'queued_sub_topics'):
					sub_topic = topic
					if topic in self._sub_topic_domains:
						# overwrite domain for this specific topic and service instance
						sub_topic += self._sub_topic_domains[topic]
					else:
						sub_topic += domain_str
					sub_callbacks.append((URI(sub_topic).uri, func_inst))
