...
class JSONLookupDomain(Domain):
    ...
    def __init__(self, name: str, json_ontology_file: str = None, sqllite_db_file: str = None):
        ...
        json_ontology_file = json_ontology_file or os.path.join('resources', 'databases',
                                                                name + '-rules.json')
    ...
    def get_discourse_acts(self):
        """ Returns the possible discourse acts """
        return self.ontology_json['discourseAct']

    def get_methods(self):
        """ Returns if the user e.g. informs by constraints or by primary key, if they restart the
            dialog, ...) """
        return self.ontology_json['method']
